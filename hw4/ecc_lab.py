from vec import Vec
from mat import Mat
from bitutil import noise, str2bits, bits2str,bits2mat, mat2bits 
from GF2 import one
...
def find_error(e):
    ...
    """
    return Vec(H.D[1], {k: one for (k,v) in  mat2coldict(H).items() if v == e})     

## Task 4 part 2
# Use the Vec class for your answers.
non_codeword = Vec({0,1,2,3,4,5,6}, {0: one, 1:0, 2:one, 3:one, 4:0, 5:one, 6:one})
error_vector = find_error(H * non_codeword) #Vec(..., ...)
code_word = non_codeword - error_vector # Vec(..., ...)
...
bits_after  = len(C.D[1]) * len(C.D[0])
